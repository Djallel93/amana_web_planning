...
> **`QUEUE_CONNECTION=sync`** : aucun worker ni cron job nécessaire. Les emails SMTP sont envoyés directement lors de chaque action (validation de candidature, invitation…). Le délai ajouté est négligeable (< 2 secondes).
