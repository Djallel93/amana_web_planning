...
function applyDefaultFilters(): void {
    ...
    // chargées (allYears/allMonths). Sans cette restriction, un mois "voisin"
    // calculé à partir de la date du jour (ex. juin) peut n'avoir aucune
    // semaine correspondante — il reste alors invisible dans la barre de
    // filtres (aucun bouton ne le représente) mais demeure dans le Set actif.
    // Résultat : décocher tous les boutons visibles ne vide jamais totalement
    // le Set, qui garde ce membre fantôme, et le filtre se met à exiger une
    // correspondance sur un mois/année qui n'existe dans aucune semaine —
    // faisant disparaître tous les résultats au lieu d'afficher "tout".
...
async function deleteCreneau(creneauId: number): Promise<void> {
    if (
        !(await ask({
            message: "Supprimer ce créneau et toutes ses tâches ?",
            danger: true,
        }))
    )
        return;
...
async function deleteWeek(semaine: SemaineData): Promise<void> {
    ...
    if (
        !(await ask({
            message: `Supprimer les ${ids.length} créneaux de cette semaine ?`,
            danger: true,
        }))
    )
        return;
    ...
    for (const id of ids) {
        try {
            ...
            /* on continue malgré une erreur isolée, comme l'original */
        }
    }
...
<template>
    <div>
        ...
        <template v-else>
            ...
            <!-- Blocs semaine -->
            <div
                v-for="semaine in semainesVisibles"
                :key="semaine.cle"
            ...
            >
                ...
                >
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <span
                            ...
                            class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-rose-500/20 border border-rose-500/40 text-rose-300"
                ...
                <template v-for="(b, i) in semaine.bannieres" :key="i">
                    <div
                        v-if="b.informatif"
                        class="flex items-center gap-2 px-4 py-2 bg-sky-50 border-b border-sky-100 text-[12.5px] text-sky-800"
                    >
                        <span>📅</span><span class="font-bold">{{ b.nom }}</span
                        ><span class="opacity-70">— {{ b.dateLabel }}</span>
                    </div>
                    <div
                        v-else
                        class="flex flex-wrap items-center gap-2 px-4 py-2 bg-rose-50 border-b border-rose-100 text-[12.5px] text-rose-800"
                    >
                        <span>🚫</span><span class="font-bold">{{ b.nom }}</span
                        ><span class="opacity-70">— {{ b.dateLabel }}</span>
                        <div class="flex flex-wrap gap-1 ml-1">
                            <span
                                v-for="tb in b.tachesBloquees"
                                :key="tb.code"
                ...
                <div class="hidden md:block overflow-x-auto">
                    ...
                    >
                        <thead>
                            <tr>
                                <th
                                    class="text-left px-4 py-2.5 text-[10.5px] font-bold text-ink-muted uppercase tracking-[0.7px] bg-surface-2 border-b border-surface-3 whitespace-nowrap font-body w-36"
                                >
                                    Jour
                                </th>
                                <th
                                ...
                                <th
                                    class="text-left px-3 py-2.5 text-[10.5px] font-bold text-ink-muted uppercase tracking-[0.7px] bg-surface-2 border-b border-surface-3 font-body"
                                >
                                    Événements
                                </th>
                                <th
                            ...
                            >
                                <td class="px-4 py-2.5">
                                    ...
                                    >
                                        ...
                                        >
                                        <span
                                            v-else-if="creneau.partielBloque"
                                            class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-700"
                                            >● Partiel</span
                                        >
                                    </div>
                                </td>
                                ...
                                <td
                                    v-for="tache in creneau.taches"
                                    :key="tache.code"
                                    class="px-2 py-2 relative"
                                    :class="tache.bloquee ? 'bg-orange-50' : ''"
                                >
                                    <div
                                        v-if="tache.bloquee"
                                        class="flex items-center gap-1 px-2 py-1 rounded-md cursor-default"
                                    >
                                        <span
                                    ...
                                    >
                                        <span
                                            v-if="tache.personne"
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11.5px] font-semibold"
                ...
                <div class="md:hidden divide-y divide-surface-3">
                    <div
                        ...
                        :key="creneau.id"
                        class="px-4 py-3"
                        :class="creneau.toutBloque ? 'bg-orange-50' : ''"
                    >
                        <div class="flex items-center justify-between mb-2.5">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                <span
                                    v-else-if="creneau.partielBloque"
                                    class="px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-700"
                                    >● Partiel</span
                                >
                            </div>
                            <button
