<!-- resources/js/Pages/Guide/Index.vue -->
<!--
    ...
    ── Accordéon <details> ──────────────────────────────────────────────────
    Conservé tel quel : HTML natif, aucune logique Vue pour l'ouverture/
    fermeture (choix déjà documenté dans le Blade d'origine — pas de JS
    requis).

    ── Sommaire (défilement doux + lien actif) ──────────────────────────────
    Le Blade d'origine poussait un <script> classique via @push('scripts')
    dans #mainWrapper (voir amana-shared::layouts.partials.app). Cette pile
    n'existe plus pour une page Inertia (resources/views/app.blade.php ne
    fait pas de @stack('scripts')) : la même logique, verbatim, tourne
    maintenant dans onMounted() ci-dessous.
-->
...
defineProps<{
    // Route::has('settings.index') calculé côté serveur (voir
    // GuideController) — Vue ne peut pas appeler Route::has() côté client,
    // ce prop reproduit fidèlement le même test que le Blade d'origine.
...
// Même gating que layouts/partials/sidebar.blade.php et l'ancien Blade de
// cette page : section Gestion visible pour gestionnaire ET admin, section
// Administration réservée à admin seul.
const isGestionOuAdmin = computed(
    () =>
        !!page.props.auth.user?.isGestionnaire ||
        !!page.props.auth.user?.isAdmin,
);
const isAdmin = computed(() => !!page.props.auth.user?.isAdmin);

onMounted(() => {
    // Défilement doux au clic sur un lien du sommaire, sans dépendre de
    // "scroll-behavior: smooth" global (qui affecterait tout le site).
    document
        .querySelectorAll<HTMLAnchorElement>("#guideSommaire [data-guide-link]")
        .forEach((link) => {
...
<template>
    <Head title="Guide d'utilisation — AMANA" />
    ...
    Chaque point est un <details> (accordéon natif, pas de JS requis pour
        ...
        <nav aria-label="Sommaire du guide" id="guideSommaire" class="sticky top-4 z-10 bg-surface border border-surface-border rounded-xl shadow-sm px-4 py-3 mb-7
            ...
            <a href="#bilan" data-guide-link
                class="guide-nav-link text-[12.5px] font-semibold text-accent px-2.5 py-1 rounded-md transition-colors duration-200 no-underline hover:bg-accent/10">🧾
                Bilan</a>
            <a v-if="isGestionOuAdmin" href="#gestion" data-guide-link
                class="guide-nav-link text-[12.5px] font-semibold text-accent px-2.5 py-1 rounded-md transition-colors duration-200 no-underline hover:bg-accent/10">⚙️
                Gestion</a>
            <a v-if="isAdmin" href="#administration" data-guide-link
                class="guide-nav-link text-[12.5px] font-semibold text-accent px-2.5 py-1 rounded-md transition-colors duration-200 no-underline hover:bg-accent/10">🛡️
                Administration</a>
        </nav>
        ...
        <div class="flex flex-col gap-7">
            ...
            ══════════════════════════════════════════════════════════════════ -->
            <section v-if="isGestionOuAdmin" id="gestion" class="scroll-mt-20 bg-surface rounded-xl border border-surface-border shadow-sm overflow-hidden
                ...
                <div class="px-5 py-4 flex flex-col gap-2.5">
                    ...
                    <details v-if="hasSettingsRoute" class="guide-item group">
                        <summary class="guide-summary">
                            <span class="guide-summary-icon bg-accent/10">⚙️</span>
            ...
            ══════════════════════════════════════════════════════════════════ -->
            <section v-if="isAdmin" id="administration" class="scroll-mt-20 bg-surface rounded-xl border border-surface-border shadow-sm overflow-hidden
...
/* ── Styles locaux au guide d'utilisation ──────────────────────────
   Repris tels quels depuis le @push('scripts') du Blade d'origine (qui,
   malgré son nom, contenait un <style> global — regroupé là plutôt qu'en
   classes utilitaires répétées sur chaque <details> pour garder le Blade
   lisible malgré le nombre de blocs). "scoped" ici est sans risque : ces
   classes (.guide-item, .guide-summary, etc.) ne sont utilisées que dans
   cette page. */
...
/* Sommaire : lien actif mis en avant pendant le défilement (voir onMounted ci-dessus) */
