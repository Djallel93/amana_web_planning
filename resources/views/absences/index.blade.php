                    ...
                    <tbody>
                        @forelse($absences as $absence)
                                        @php
                                            ...
                                            $canDelete = auth()->user()->isAdmin() || $isMine;
                                        @endphp
                                        <tr class="{{ $isMine ? 'my-absence-row' : '' }}">
                                            <td>
                                                <div style="display:flex; align-items:center; gap:10px;">
                                                    <div style="
                                                            width:32px; height:32px;
                                                            background:linear-gradient(135deg,
                                                                {{ $isMine ? 'var(--primary),var(--violet)' : '#6b7280,#9ca3af' }});
                                                            border-radius:50%;
                                                    ...
                                                    <div>
                                                        <div class="td-primary" style="{{ $isMine ? 'color:var(--primary)' : '' }}">
                                                            {{ $absence->personne
                            ? $absence->personne->prenom . ' ' . $absence->personne->nom
                            : 'Inconnu' }}
                                                            @if($isMine)
                                                                <span
                                                                    style="font-size:11px; font-weight:400; color:var(--primary); margin-left:4px;">(moi)</span>
                                                            @endif
                                                        </div>
                                                        @if($actuelle)
                                            ...
                                            <td>
                                                {{-- Bouton supprimer : visible si admin OU si c'est sa propre absence --}}
                                                @if($canDelete)
                                ...
                                <td colspan="6">
                                    <div class="empty-state" style="padding:36px;">
                                        ...
                                        <div class="empty-desc">
                                            Ajoutez une absence via le formulaire ci-contre.
                                        </div>
                                    </div>
                                </td>
                            </tr>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('absences.store') }}" method="POST">
                    ...
                    {{-- Sélecteur de personne : admin voit tout le monde, membre voit seulement lui --}}
                    <div class="form-group" style="margin-bottom:16px;">
                        ...
                        @else
                            {{-- Membre : champ caché avec son propre ID --}}
                    ...
                    <button type="submit" class="btn btn-primary" style="width:100%; justify-content:center;">
                        ➕ {{ auth()->user()->isAdmin() ? 'Ajouter l\'absence' : 'Déclarer mon absence' }}
                    </button>
                </form>
            </div>
        </div>
