                    ...
                    <div style="padding:20px 24px;">
                        <div style="display:flex; align-items:flex-start; justify-content:space-between; gap:16px; flex-wrap:wrap;">
                            ...
                            <div style="display:flex; flex-direction:column; gap:6px; min-width:180px;">
                                <div
                                    style="font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:0.6px; color:var(--ink-muted);">
                                    Informations
                                </div>
                        ...
                        : '—' }}
                            ...
                            <div style="min-width:220px;">
                                <div
                                    style="font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:0.6px; color:var(--ink-muted); margin-bottom:8px;">
                                    Disponibilités déclarées
                                </div>
                                @php
                                    $jours = ['Vendredi', 'Samedi'];
                                    $hasAnyRestriction = $candidat->restrictions->isNotEmpty();
                                @endphp

                                @if(!$hasAnyRestriction)
                                    <span style="font-size:12px; color:var(--ink-faint); font-style:italic;">
                                        Non renseignées
                                    </span>
                                @else
                                    <div style="display:flex; flex-direction:column; gap:4px;">
                                        @foreach($jours as $jour)
                                            @php
                                                $restrictionsDuJour = $candidat->restrictions->where('jour', $jour);
                                            @endphp
                                            @if($restrictionsDuJour->isNotEmpty())
                                                <div style="display:flex; align-items:center; gap:6px; flex-wrap:wrap;">
                                                    <span
                                                        style="font-size:12px; font-weight:600; color:var(--ink-muted); width:58px;">{{ $jour }}</span>
                            ...
                            <div style="display:flex; flex-direction:column; gap:8px; flex-shrink:0; min-width:200px;">
                                ...
                                {{-- Valider avec sélection du rôle --}}
                                <form action="{{ route('admin.candidatures.valider', $candidat->id) }}" method="POST"
                                    ...
                                    <div style="margin-bottom:8px;">
                                        <label style="
                                                            ...
                                                            letter-spacing:0.6px; color:var(--ink-muted); display:block; margin-bottom:5px;
                                                        ">
                                            Rôle attribué
                                        </label>
...
@push('scripts')
    <script>
        function confirmValidation(form) {
            const roleLabel = form.querySelector('select[name="role"]').options[
                form.querySelector('select[name="role"]').selectedIndex
            ].text.trim();
            return confirm(`Valider cette candidature avec le rôle "${roleLabel}" ?\nUn email d'invitation sera envoyé.`);
        }
    </script>
@endpush